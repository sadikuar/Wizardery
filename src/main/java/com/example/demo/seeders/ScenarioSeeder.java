...
public class ScenarioSeeder implements TableSeeder{
		...
		if(scenarioRepository.findAll().isEmpty()) {
			...
			for(int i=0; i<getMinTabLength();i++) {
				...
				if(optionalRpg.isPresent()) {
					...
					Scenario scenario = createScenario(tabNames[i], tabDescriptions[i], tabQuests[i], tabDifficulties[i], tabMinPlayers[i], tabAdvisedPlayers[i], tabMaxPlayers[i], tabTimeApprox[i], rpg, creator);
	...
	private Scenario createScenario(String name, String description, String quests, String difficulty, int minPlayers, int advisedPlayers, int maxPlayers,double timeApprox, Rpg rpg, User creator) {
